...
const fuzzy = (object = {}) => {
  ...
  return new Proxy(object, {
    get(target, prop, reciever) {
      if (typeof prop === 'symbol') return Reflect.get(target, prop, reciever);

      if (isNumber(prop)) {
        return fuzzy(Reflect.get(target, closestIndex(target, prop), reciever));
      }

      return fuzzy(Reflect.get(target, closestMatch(target, prop), reciever));
    },
